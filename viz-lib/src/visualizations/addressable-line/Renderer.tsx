...
export default function Renderer(input: any) {
  ...
  return <LineChart data={data} columns={columns} />;
}

function LineChart({ data, columns }: any) {
  ...
  useEffect(() => {
    ...
    const sampleData = data["discountPercentage"].data;
    const { xScale, yScale } = createScales(width, height, sampleData);
    const selectedColumns = selectedOptions.map(i => i.value);

    console.log(sampleData);

    createLineChartAxis(g, xScale, yScale, height);
    const chartArea = createLineChart(g, xScale, yScale, width, data, selectedColumns);
    createLineChartGradients(g, width, height);
    createLineChartCursor(tooltipRef, setTooltipData, chartArea, xScale, yScale, height, sampleData, data, columns);
...
function createLineChartGradients(
  g: d3.Selection<SVGGElement, unknown, null, undefined>,
  width: number,
  height: number
) {
...
function createLineChart(
  ...
  chartArea
    ...
    .style("fill", "none")
    // @ts-ignore
    .attr("d", createChartLine(data["discountPercentage"].data));
...
function createLineChartCursor(
