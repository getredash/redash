...
export default function ColumnsSettings({ options, onOptionsChange }: any) {
  ...
  return (
    <SortableContainer
      ...
      lockAxis="y"
      useDragHandle
      helperClass="details-editor-columns-dragged-item"
      helperContainer={(container: any) => container.firstChild}
      onSortEnd={handleColumnsReorder}
      containerProps={{
        className: "details-visualization-editor-columns",
      }}>
        ...
        {map(options.columns, (column, index) => (
          <SortableItem
            ...
            header={
              <React.Fragment>
                <DragHandle />
                <span data-test={`Details.Column.${column.name}.Name`}>
                  {column.name}
            ...
            extra={
              <Tooltip title="Toggle visibility" mouseEnterDelay={0} mouseLeaveDelay={0}>
                {column.visible ? (
                  <EyeOutlinedIcon
                    data-test={`Details.Column.${column.name}.Visibility`}
