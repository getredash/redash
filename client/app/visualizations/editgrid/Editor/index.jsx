...
function getDefaultColumnSettings(tableColumns, dataColumns, specialColumns, optionsChanged) {
  const result = {};
  const dataColumnNames = dataColumns.map(col => col.name);
  tableColumns.forEach((col) => {
    const columnType = includes(dataColumnNames, col) ? find(dataColumns, c => c.name === col).type : 'string';
    result[col] = {
      name: col,
      type: columnType,
      table: true,
...
export default function Editor(props) {
  ...
  const tableColumns = options.editTable ? find(schema, t => t.name === options.editTable).columns : [];
  const commonColumns = tableColumns ? data.columns.filter(c => includes(tableColumns, c.name)) : [];
  ...
  if (options.editTable && (!options.columns || isEqual(options.columns, {}))) {
    getDefaultColumnSettings(tableColumns, data.columns, specialColumns, optionsChanged);
  }
  ...
  return (
    <Tabs animated={false} tabBarGutter={0}>
      <Tabs.TabPane key="general" tab={<span data-test="EditGrid.EditorTabs.General">General</span>}>
        <GeneralSettings
          {...props}
