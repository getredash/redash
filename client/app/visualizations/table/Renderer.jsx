...
export default function Renderer({ options, data, context }) {
  const [rowKeyPrefix, setRowKeyPrefix] = useState(
    `row:1:${options.itemsPerPage}:`
  );
  const [searchTerm, setSearchTerm] = useState("");
  const [orderBy, setOrderBy] = useState([]);

  const searchColumns = useMemo(() => filter(options.columns, "allowSearch"), [
    options.columns
  ]);
      ...
      <Table
        ...
        pagination={{
          ...
          onChange: (page, pageSize) =>
            setRowKeyPrefix(`row:${page}:${pageSize}:`)
        }}
      />
    </div>
  );
