...
const VisualizationConfig = PropTypes.shape({
  type : PropTypes.string.isRequired,
  name : PropTypes.string.isRequired,
  getOptions : PropTypes.func.isRequired, // (existingOptions: object, data: {
                                          // columns[], rows[] }) => object
...
export function getDefaultVisualization() {
  // return any visualization explicitly marked as default, or any
  // non-deprecated otherwise
  return find(registeredVisualizations,
              visualization => visualization.isDefault) ||
         find(registeredVisualizations,
              visualization => !visualization.isDeprecated);
}

export function newVisualization(type = null, options = {}) {
  const visualization =
      type ? registeredVisualizations[type] : getDefaultVisualization();
  return {
