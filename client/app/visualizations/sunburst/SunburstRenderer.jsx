...
function drawSunburst(rows, element) {
  // svg dimensions
  ...
  function buildNodes(raw) {
    ...
    if (
      ...
      const grouped = groupBy(raw, 'sequence');
      ...
      values = map(grouped, (value) => {
        ...
        return {
          ...
          nodes: map(sorted, 'node'),
        };
      });
    } else {
      const validKey = key => key !== 'value' && key.indexOf('$$') !== 0;
      const ks = sortBy(filter(keys(raw[0]), validKey), identity);

      values = map(raw, (row, sequence) => ({
        size: row.value,
        sequence,
        nodes: compact(map(ks, key => row[key])),
      }));
    }
