...
// Plotly examples and docs: https://plot.ly/javascript/
    ...
    const refreshColumns = () => {
      ...
      if (this.columnNames.length > 0) {
        each(
          difference(keys(this.options.columnMapping), this.columnNames),
          column => {
    ...
    const refreshColumnsAndForm = () => {
      ...
      if (data && data.columns.length > 0 && data.rows.length > 0) {
        this.form.yAxisColumns = intersection(
    ...
    const refreshSeries = () => {
      ...
      if (this.options.globalSeriesType === "pie") {
        ...
        const newValues = difference(
          valuesNames,
          keys(this.options.valuesOptions)
        );
        each(newValues, name => {
