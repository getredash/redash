...
export default function applyLayoutFixes(plotlyElement, layout, updatePlot) {
  ...
  if (layout.width <= 600) {
    ...
    updatePlot(plotlyElement, pick(layout, [ 'width', 'height', 'legend' ]))
        .then(() => {
          const legend = plotlyElement.querySelector(
              '.legend'); // eslint-disable-line no-shadow
          if (legend) {
            // compute real height of legend - items may be split into few
            // columnns, also scrollbar may be shown
            const bounds =
                reduce(legend.querySelectorAll('.traces'), (result, node) => {
            ...
            //    plot; if legend is too large, plotly will reduce it's height
            //    and show a scrollbar; in this case, height of plot === height
            //    of legend, so we can split container's height half by half
            //    between them.
            ...
            // offset the legend
            legend.style[transformName] =
                'translate(0, ' + layout.height + 'px)';
    ...
    layout.legend = {
      orientation : 'v',
      // vertical legend will be rendered properly, so just place it to the
      // right side of plot
      y : 1,
      x : 1,
      xanchor : 'left',
