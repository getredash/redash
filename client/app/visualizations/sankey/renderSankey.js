import { isNil, map, extend, sortBy, includes, filter, reduce, keys, values, identity } from 'lodash';
import d3 from 'd3';
...
export default function renderSankey(element, data) {
  const margin = {
    top: 10,
    right: 10,
    bottom: 10,
  ...
  data = graph(data);
  data.nodes = map(data.nodes, d => extend(d, { color: color(d.name.replace(/ .*/, '')) }));
