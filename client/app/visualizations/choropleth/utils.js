...
export function darkenColor(color) { return chroma(color).darken().hex(); }
...
export function prepareFeatureProperties(feature, valueFormatted, data,
                                         countryCodeType) {
  const result = {};
  _.each(feature.properties, (value, key) => { result['@@' + key] = value; });
...
export function createScale(features, data, options) {
  ...
  const values = _.uniq(_.filter(
      _.map(features, feature => getValueForFeature(feature, data,
