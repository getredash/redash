...
export const lightingEffect =
    new LightingEffect({ambientLight, pointLight1, pointLight2});
...
export function setMidLatLon(data, options, onOptionsChange) {
  const lat =
      data.reduce(
          (a, b) => ({
            [options.latColName] : a[options.latColName] + b[options.latColName]
          }),
  ...
  const lon =
      data.reduce(
          (a, b) => ({
            [options.lonColName] : a[options.lonColName] + b[options.lonColName]
          }),
  ...
  if (onOptionsChange) {
    onOptionsChange(extend(
        options, {centerLat : lat, centerLon : lon, isCenterSet : true}));
  }
