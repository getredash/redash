...
function _renderLayers(data, options, setTooltip, selectedLayer) {
  ...
  const onHovered = ({ x, y, object }) =>
    setTooltip({
      x,
      y,
      count: object ? object.points.length : 0,
      displayNames: object
        ? [...new Set(object.points.map((o) => o[displayCol]))].join(",")
        : "",
    });

  if (
    data[0] &&
    latColName in data[0] &&
    lonColName in data[0] &&
    groupByCol in data[0]
  ) {
    const domain = setMaxDomain ? [0, maxCount] : null;

    const mapLayers = layers.map(
      (l) =>
        new HexagonLayer({
          id: `map-${l.layername}`,
          colorRange,
    ...
    <div className="layermap-visualization-container">
      ...
      >
        ...
        {tooltipData.count ? (
          <div
            className="tooltip"
            style={{ top: tooltipData.y, left: tooltipData.x, opacity: 1 }}
          >
            <b>
