...
function isDateParameter(paramType) {
  return includes(
    ["date", "datetime-local", "datetime-with-seconds"],
    paramType
  );
}
...
export class Parameter {
  ...
  get hasPendingValue() {
    ...
    const pendingValue = isEmptyValue(this.pendingValue)
      ? null
      : this.pendingValue;
...
class Parameters {
  ...
  updateParameters(update) {
    ...
    this.query.options.parameters = parameters
      ...
      .map((p) =>
        p instanceof Parameter ? p : new Parameter(p, this.query.id)
      );
  }
...
function QueryResource(
  ...
  QueryService.prototype.prepareQueryResultExecution = function prepareQueryResultExecution(
    execute,
    maxAge
  ) {
  ...
  QueryService.prototype.getCostOnBigQuery = function getCostOnBigQuery(
    selectedQueryText
  ) {
