    ...
    _.each(queryFilters, (queryFilter) => {
      ...
      if (!_.has(filters, queryFilter.name)) {
        ...
        filters[filter.name].values =
            _.union(filters[filter.name].values, filter.values);
      }
    });
  });
...
function prepareWidgetsForDashboard(widgets) {
  ...
  // This value should be big enough so auto-height widgets will not overlap
  // other ones.
  const defaultWidgetSizeY =
      Math.max(
          _.chain(widgets).map(w => w.options.position.sizeY).max().value(),
          20,
          ) +
      5;
...
function calculateNewWidgetPosition(existingWidgets, newWidget) {
  const width = _.extend({sizeX : dashboardGridOptions.defaultSizeX},
                         _.extend({}, newWidget.options).position)
                    .sizeX;

  // Find first free row for each column
  const bottomLine =
      _.chain(existingWidgets)
          .map((w) => {
...
function DashboardService($resource, $http, $location, currentUser) {
  ...
  const resource = $resource(
      ...
      {slug : '@slug'},
      {
        ...
        query :
            {method : 'GET', isArray : false, transformResponse : transform},
        recent : {
          method : 'get',
          isArray : true,
  ...
  resource.prototype.canEdit =
      function canEdit() { return currentUser.canEdit(this) || this.can_edit; };
  ...
  resource.prototype.getParametersDefs = function getParametersDefs() {
    ...
    _.each(this.widgets, (widget) => {
      if (widget.getQuery()) {
        ...
        widget.getQuery().getParametersDefs(false).forEach((param) => {
    ...
    return _.values(_.each(globalParams, (param) => {
      ...
      param.fromUrlParams(
          queryParams); // try to initialize from url (may do nothing)
    }));
  };

  resource.prototype.addWidget = function addWidget(textOrVisualization,
                                                    options = {}) {
    const props = {
...
export default function init(ngModule) {
  ...
  ngModule.run(($injector) => { Dashboard = $injector.get('Dashboard'); });
}
