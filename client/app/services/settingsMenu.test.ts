...
describe("SettingsMenu", () => {
    describe("isActive", () => {
        test("works with non multi org paths", () => {
            expect((settingsMenu.getActiveItem("/data_sources/") as any).title).toBe(dataSourcesItem.title);
        });
        test("works with multi org paths", () => {
            ...
            expect((settingsMenu.getActiveItem("/acme/data_sources/") as any).title).toBe(dataSourcesItem.title);
        });
    });
});
