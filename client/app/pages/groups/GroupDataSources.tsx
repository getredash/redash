...
class GroupDataSources extends React.Component<Props> {
    ...
    listColumns = [
        ...
        Columns.custom((text: any, datasource: any) => {
            const menu = (<Menu selectedKeys={[datasource.view_only ? "viewonly" : "full"]} onClick={item => this.setDataSourcePermissions(datasource, item.key)}>
        ...
        Columns.custom((text: any, datasource: any) => (<Button className="w-100" type="danger" onClick={() => this.removeGroupDataSource(datasource)}>
          Remove
        </Button>), {
            width: "1%",
            isAvailable: () => currentUser.isAdmin,
    ...
    addDataSources = () => {
        ...
        const alreadyAddedDataSources = map((this.props.controller as any).allItems, ds => ds.id);
        SelectItemsDialog.showModal({
            ...
            renderItem: (item: any, { isSelected }: any) => {
                ...
                return {
                    content: (<DataSourcePreviewCard dataSource={item}>
    ...
    render() {
        ...
        <Layout>
          ...
          <Layout.Sidebar>
            ...
            <Sidebar controller={controller} group={this.group} items={this.sidebarMenu} canAddDataSources={currentUser.isAdmin} onAddDataSourcesClick={this.addDataSources} onGroupDeleted={() => navigateTo("groups")}/>
          </Layout.Sidebar>
          {/* @ts-expect-error ts-migrate(2746) FIXME: This JSX tag's 'children' prop expects a single ch... Remove this comment to see the full error message */}
          <Layout.Content>
            ...
            {controller.isLoaded && controller.isEmpty && (<div className="text-center">
                ...
                {currentUser.isAdmin && (<Button type="primary" onClick={this.addDataSources}>
...
const GroupDataSourcesPage = wrapSettingsTab("Groups.DataSources", null, itemsList(GroupDataSources, () => new ResourceItemsSource({
    isPlainList: true,
...
routes.register("Groups.DataSources", routeWithUserSession({
