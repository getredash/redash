...
export default function useAddQueryToRecentQueries(query) {
    useEffect(() => {
        ...
        if(!currentRecentQueries) {
            ...
            const recentQueriesIncludesCurrentQuery = find(parsedCurrentRecentQueries, (recentQuery) => recentQuery.id === query.id) !== undefined;
            const biggestPriorityValue = reduce(currentRecentQueriesPrioritys, (a, b) => Math.max(a, b));
            if(recentQueriesIncludesCurrentQuery) {
                const newRecentQueries = map(parsedCurrentRecentQueries, (recentQuery) => recentQuery.id === query.id ? {id: query.id, priority: biggestPriorityValue + 1} : recentQuery);
