...
class AlertsList extends React.Component<Props> {
    listColumns = [
        (Columns.custom as any).sortable((text: any, alert: any) => <i className={`fa fa-bell-${alert.options.muted ? "slash" : "o"} p-r-0`}/>, {
            title: <i className="fa fa-bell p-r-0"/>,
            field: "muted",
            width: "1%",
        }),
        (Columns.custom as any).sortable((text: any, alert: any) => (<div>
        ...
        (Columns.timeAgo as any).sortable({ title: "Last Updated At", field: "updated_at", width: "1%" }),
        (Columns.dateTime as any).sortable({ title: "Created At", field: "created_at", width: "1%" }),
    ];
    render() {
        const { controller } = this.props;
        return (<div className="page-alerts-list">
        <div className="container">
          ...
          <PageHeader title={controller.params.pageTitle} actions={currentUser.hasPermission("list_alerts") ? (<Link.Button block type="primary" href="alerts/new">
                  <i className="fa fa-plus m-r-5"/>
                  New Alert
                </Link.Button>) : null}/>
          <div>
            {controller.isLoaded && controller.isEmpty ? (<DynamicComponent name="AlertsList.EmptyState">
                {/* @ts-expect-error ts-migrate(2769) FIXME: No overload matches this call. */}
                <EmptyState icon="fa fa-bell-o" illustration="alert" description="Get notified on certain events" helpMessage={<EmptyStateHelpMessage helpTriggerType="ALERTS"/>} showAlertStep/>
              </DynamicComponent>) : (<div className="table-responsive bg-white tiled">
                {/* @ts-expect-error ts-migrate(2769) FIXME: No overload matches this call. */}
                <ItemsTable loading={!controller.isLoaded} items={controller.pageItems} columns={this.listColumns} orderByField={controller.orderByField} orderByReverse={controller.orderByReverse} toggleSorting={controller.toggleSorting}/>
...
const AlertsListPage = itemsList(AlertsList, () => new ResourceItemsSource({
    isPlainList: true,
    getRequest() {
        return {};
    },
...
routes.register("Alerts.List", routeWithUserSession({
    path: "/alerts",
    title: "Alerts",
