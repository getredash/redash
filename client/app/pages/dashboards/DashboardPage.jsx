    ...
    <div className={cx("add-widget-container", className)} {...props}>
      ...
      <div>
        ...
        <Button type="primary" onClick={showAddWidgetDialog} data-test="AddWidgetButton">
          Add Widgetimport { isEmpty, map } from "lodash";
import React, { useState, useEffect } from "react";
import PropTypes from "prop-types";
import cx from "classnames";
...
function DashboardComponent(props) {
  ...
  useEffect(() => {
    ...
    Auth.loadSession().then(({ client_config, user }) => { setWatermarkEnabled(client_config.watermarkEnabled); setCurrentUser(user) });

  }, [watermarkEnabled, currentUser]);
  ...
  return (
    <div className="container" ref={setPageContainer} data-test={`DashboardId${dashboard.id}Container`}>
...

      {
        ...
        watermarkEnabled && currentUser && <Watermark content={[currentUser.name, currentUser.email]} {...watermarkOptions}>
          {dashboardGrid}
        </Watermark>
      }
      {
