...
export default function SAMLSettings(props: any) {
    ...
    const onChangeEnabledStatus = (e: any) => {
        ...
        if (e.target.value) {
            (updates as any).auth_saml_type = e.target.value;
        }
        onChange(updates);
    };
    return (<DynamicComponent name="OrganizationSettings.SAMLSettings" {...props}>
      <h4>SAML</h4>
      <Form.Item label="SAML Enabled">
        {loading ? (<Skeleton title={{ width: 300 }} paragraph={false} active/>) : (<Radio.Group onChange={onChangeEnabledStatus} value={values.auth_saml_enabled && (values.auth_saml_type || "dynamic")}>
      ...
      {values.auth_saml_enabled && (<>
          {values.auth_saml_type === "static" && (<>
              ...
              <Form.Item label="SAML x509 cert">
                <Input value={values.auth_saml_x509_cert} onChange={e => onChange({ auth_saml_x509_cert: e.target.value })}/>
              </Form.Item>
            </>)}
          {values.auth_saml_type === "dynamic" && (<>
              <Form.Item label="SAML Metadata URL">
                <Input value={values.auth_saml_metadata_url} onChange={e => onChange({ auth_saml_metadata_url: e.target.value })}/>
              </Form.Item>
              <Form.Item label="SAML Entity ID">
                <Input value={values.auth_saml_entity_id} onChange={e => onChange({ auth_saml_entity_id: e.target.value })}/>
              </Form.Item>
              <Form.Item label="SAML NameID Format">
                <Input value={values.auth_saml_nameid_format} onChange={e => onChange({ auth_saml_nameid_format: e.target.value })}/>
              </Form.Item>
            </>)}
        </>)}
    </DynamicComponent>);
