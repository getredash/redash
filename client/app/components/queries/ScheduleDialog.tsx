...
export function TimeEditor(props: TimeEditorProps) {
    ...
    return (<React.Fragment>
      ...
      {showUtc && (<span className="utc" data-testid="utc">
...
class ScheduleDialog extends React.Component<ScheduleDialogProps, ScheduleDialogState> {
    ...
    setInterval = (newSeconds: any) => {
        ...
        if (newInterval === IntervalEnum.WEEKS && !this.state.dayOfWeek) {
            (newSchedule as any).day_of_week = WEEKDAYS_FULL[0];
        }
        (newSchedule as any).interval = newSeconds;
        const [hour, minute] = (newSchedule as any).time ? localizeTime((newSchedule as any).time).split(":") : [null, null];
        this.setState({
            interval: newInterval,
            seconds: newSeconds,
            hour,
            minute,
            dayOfWeek: (newSchedule as any).day_of_week ? WEEKDAYS_SHORT[WEEKDAYS_FULL.indexOf((newSchedule as any).day_of_week)] : null,
        });
    ...
    save() {
        ...
        // save if changed
        if (hasChanged()) {
            if ((newSchedule as any).interval) {
    ...
    render() {
        ...
        return (<Modal {...dialog.props} title="Refresh Schedule" className="schedule" onOk={() => this.save()}>
        <div className="schedule-component">
          ...
          <div data-testid="interval">
            ...
            .map(int => (<OptGroup label={capitalize(pluralize(int))} key={int}>
                    ...
                    {this.intervals[int].map(([cnt, secs]) => (<Option value={secs} key={`${int}-${cnt}`}>
        ...
        {[IntervalEnum.DAYS, IntervalEnum.WEEKS].indexOf(interval) !== -1 ? (<div className="schedule-component">
            <h5>On time</h5>
            <div data-testid="time">
              <TimeEditor defaultValue={hour
            ? moment()
                // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'string' is not assignable to par... Remove this comment to see the full error message
                .hour(hour)
                // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'string | null' is not assignable... Remove this comment to see the full error message
                .minute(minute)
            : null} onChange={this.setTime}/>
            </div>
          </div>) : null}
        {IntervalEnum.WEEKS === interval ? (<div className="schedule-component">
            <h5>On day</h5>
            <div data-testid="weekday">
              ...
              <Radio.Group size="medium" defaultValue={this.state.dayOfWeek} onChange={this.setWeekday}>
                {WEEKDAYS_SHORT.map(day => (<Radio.Button value={day} key={day} className="input">
                    {day[0]}
                  </Radio.Button>))}
              </Radio.Group>
            </div>
          </div>) : null}
        {interval !== IntervalEnum.NEVER ? (<div className="schedule-component">
            <h5>Ends</h5>
            <div className="ends" data-testid="ends">
              ...
              {until ? (<DatePicker size="small" className="datepicker" value={moment(until)} allowClear={false} format={DATE_FORMAT} onChange={this.setScheduleUntil}/>) : null}
            </div>
          </div>) : null}
      </Modal>);
    }
