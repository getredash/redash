...
function orderedInputs(properties: any, order: any, targetOptions: any) {
    ...
    Object.keys(properties).forEach(key => {
        ...
        if (input.type === "select") {
            ...
            (input as any).options = properties[key].options;
        }
        if (position > -1) {
...
function getFields(type = {}, target = { options: {} }) {
    const configurationSchema = (type as any).configuration_schema;
    ...
    const isNewTarget = !(target as any).id;
    const inputs = [
        {
            name: "name",
            title: "Name",
            type: "text",
            required: true,
            initialValue: (target as any).name,
            contentAfter: React.createElement("hr"),
            placeholder: `My ${(type as any).name}`,
...
function getBase64(file: any) {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = () => resolve((reader.result as any).substr((reader.result as any).indexOf(",") + 1));
