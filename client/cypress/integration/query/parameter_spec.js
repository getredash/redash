...
describe('Parameter', () => {
  const expectDirtyStateChange = (edit) => {
    cy.getByTestId('ParameterName-test-parameter')
        ...
        .should(($el) => { assert.isUndefined($el.data('dirty')); });

    edit();

    cy.getByTestId('ParameterName-test-parameter')
        .find('.parameter-input')
        .should(($el) => { assert.isTrue($el.data('dirty')); });
  };

  const expectValueValidationError = (edit, expectedInvalidString =
                                                'Required parameter') => {
    cy.getByTestId('ParameterName-test-parameter')
        ...
        .should(
            'not.have.class',
            'show-help-enter'); // assures ant animation ended for screenshot
  };
  ...
  describe('Text Parameter', () => {
    ...
    it('shows validation error when value is empty', () => {
      cy.getByTestId('ParameterName-test-parameter').find('input').clear();
  ...
  describe('Date and Time Parameter', () => {
    ...
    it('updates the results after selecting a date and clicking in ok',
       function() {
         ...
         cy.getByTestId('TableVisualization')
             .should('contain',
                     Cypress.moment(this.now).format('YYYY-MM-15 HH:mm'));
       });
    ...
    it('shows the current datetime after clicking in Now', function() {
      ...
      cy.getByTestId('TableVisualization')
          .should('contain',
                  Cypress.moment(this.now).format('YYYY-MM-DD HH:mm'));
    });
  ...
  describe('Inline feedback', () => {
    beforeEach(function() {
      ...
      createQuery(queryData, false).then((query) => {
        this.query = query;
    ...
    it('shows unsaved feedback in query page', function() {
      ...
      cy.get('@Feedback')
          ...
          .should(
              'not.have.class',
              'show-help-appear'); // assures ant animation ended for screenshot
    ...
    it('shows validation error in visualization embed', function() {
      cy.visit(`/embed/query/${this.query.id}/visualization/${
          this.vizId}?api_key=${this.query.api_key}`);
      expectValueValidationError();
  ...
  describe('Apply Changes', () => {
    ...
    it('applies changes from "alt+enter" keyboard shortcut', () => {
      expectAppliedChanges((input) => { input.type('{alt}{enter}'); });
    });
  ...
  describe('Draggable', () => {
    ...
    it('is possible to rearrange parameters', function() {
      ...
      const expectedOrder =
          [ 'Parameter 2', 'Parameter 1', 'Parameter 4', 'Parameter 3' ];
