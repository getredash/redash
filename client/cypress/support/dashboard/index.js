...
import {addWidget, createQuery} from '../redash-api';

const {get} = Cypress._;
const RESIZE_HANDLE_SELECTOR = '.react-resizable-handle';

export function getWidgetTestId(widget) { return `WidgetId${widget.id}`; }
...
export function editDashboard() {
  cy.getByTestId('DashboardMoreMenu')
      .click()
      .within(() => { cy.get('li').contains('Edit').click(); });
}

export function shareDashboard() {
  cy.clickThrough({button : 'Publish'}, `OpenShareForm
    PublicAccessEnabled`);
