(function() {
    var QueryResult = function($resource, $timeout) {
        var QueryResultResource = $resource('/api/query_results/:id', {
            id: '@id'
        }, {
            'post': {
                'method': 'POST'
            ...
            Job.get({
                ...
                if (queryResult.getStatus() == "processing" && queryResult.job.query_result_id && queryResult.job.query_result_id != "None") {
                    QueryResultResource.get({
                        'id': queryResult.job.query_result_id
                    }, function(response) {
                        queryResult.update(response);
                    });
    ...
    var Query = function($resource, QueryResult, DataSource) {
        var Query = $resource('/api/queries/:id', {
            id: '@id'
        });
    ...
    var DataSource = function($resource) {
        var DataSourceResource = $resource('/api/data_sources/:id', {
            id: '@id'
        }, {
            'get': {
                'method': 'GET',
    ...
    var Groups = function($resource) {
        ...
        var GroupResource = $resource('/api/groups', {}, {
            'get': {
                'method': 'GET',
                'cache': true,
                'isArray': true
    ...
    var Widget = function($resource, Query) {
        var WidgetResource = $resource('/api/widgets/:id', {
            id: '@id'
        });
