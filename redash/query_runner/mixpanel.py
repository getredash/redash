            ...
            if (not isinstance(data[0], dict)) or type(data[0]) in [str, unicode]:
                ...
                for k in data[0]:
                    if type(data[0][k]) is dict:
                        for j in data[0][k]:
                            result["columns"].append({"name": k + "_" + j, "friendly_name": k + "_" + j})
                    elif type(data[0][k]) is list:
                        for index, j in enumerate(data[0][k]):
                            result["columns"].append({"name": k + "_" + str(index), "friendly_name": k + "_" + str(index)})
                    else:
                        result["columns"].append({"name": k, "friendly_name": k})
