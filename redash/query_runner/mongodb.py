...
class DyoTelemetryDB(BaseQueryRunner):
    ...
    @classmethod
    def name(cls):
        return "Dyo Telemetry DB"

    def __init__(self, configuration):
        super(DyoTelemetryDB, self).__init__(configuration)

        self.syntax = "json"

        self.db_name = settings.DYO_TELEMETRY_DATABASE_NAME

        self.is_replica_set = (
            True
            if settings.DYO_TELEMETRY_REPLICASET
            else False
        )
    ...
    def _get_db(self):
        kwargs = {}
        if self.is_replica_set:
            kwargs["replicaSet"] = settings.DYO_TELEMETRY_REPLICASET
            readPreference = settings.DYO_TELEMETRY_REPLICASET_READPREFERENCE
            if readPreference:
                kwargs["readPreference"] = readPreference
...
class DyoHubManagerDB(BaseQueryRunner):
    ...
    @classmethod
    def name(cls):
        return "Dyo Hubmanager DB"
    def __init__(self, configuration):
        super(DyoHubManagerDB, self).__init__(configuration)

        self.syntax = "json"

        self.db_name = settings.DYO_IOTHUB_DATABASE_NAME
