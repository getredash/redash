...
def _get_query_results(jobs, project_id, location, job_id, start_index):
    ...
    if not query_reply["jobComplete"]:
        time.sleep(10)
        return _get_query_results(jobs, project_id, location, job_id,
                                  start_index)
...
class BigQuery(BaseQueryRunner):
    ...
    def _get_total_bytes_processed(self, jobs, query):
        ...
        response = jobs.query(projectId=self._get_project_id(),
    ...
    def _get_job_data(self, query):
        ...
        if self.configuration.get("userDefinedFunctionResourceUri"):
            resource_uris = self.configuration[
                "userDefinedFunctionResourceUri"].split(",")
            job_data["configuration"]["query"][
                "userDefinedFunctionResources"] = map(
                    lambda resource_uri: {"resourceUri": resource_uri},
                    resource_uris)

        if "maximumBillingTier" in self.configuration:
            job_data["configuration"]["query"][
                "maximumBillingTier"] = self.configuration[
                    "maximumBillingTier"]
    ...
    def _get_query_result(self, jobs, query):
        ...
        insert_response = jobs.insert(projectId=project_id,
                                      body=job_data).execute()
        current_row = 0
        ...
        while ("rows" in query_reply
               ) and current_row < query_reply["totalRows"]:
            for row in query_reply["rows"]:
                rows.append(transform_row(row,
                                          query_reply["schema"]["fields"]))
        ...
        columns = [{
            ...
            f["name"],
            "type":
            "string" if f.get("mode") == "REPEATED" else types_map.get(
                f["type"], "string"),
        ...
        data = {
            ...
            "rows": rows,
            "metadata": {
                "data_scanned": int(query_reply["totalBytesProcessed"])
            },
        }
    ...
    def get_mbs_processed(self, query):
        ...
        try:
            processed_mb = (self._get_total_bytes_processed(jobs, query) /
                            1000.0 / 1000.0)
            error = None
    ...
    def run_query(self, query, user):
        ...
        try:
            if "totalMBytesProcessedLimit" in self.configuration:
                ...
                processedMB = (self._get_total_bytes_processed(jobs, query) /
                               1000.0 / 1000.0)
