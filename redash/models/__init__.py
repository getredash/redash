...
from .base import db, gfk_type, Column, GFKBase, SearchBaseQuery, key_types, primary_key
...
class DataSource(BelongsToOrgMixin, db.Model):
    ...
    org_id = Column(key_types("Organization"), db.ForeignKey("organizations.id"))
...
class DataSourceGroup(db.Model):
    ...
    data_source_id = Column(key_types("DataSource"), db.ForeignKey("data_sources.id"))
    data_source = db.relationship(DataSource, back_populates="data_source_groups")
    group_id = Column(key_types("Group"), db.ForeignKey("groups.id"))
...
class Query(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    data_source_id = Column(key_types("DataSource"), db.ForeignKey("data_sources.id"), nullable=True)
    data_source = db.relationship(DataSource, backref="queries")
    latest_query_data_id = Column(
        key_types("QueryResult"), db.ForeignKey("query_results.id"), nullable=True
    )
    ...
    user_id = Column(key_types("User"), db.ForeignKey("users.id"))
    user = db.relationship(User, foreign_keys=[user_id])
    last_modified_by_id = Column(key_types("User"), db.ForeignKey("users.id"), nullable=True)
...
class Favorite(TimestampMixin, db.Model):
    ...
    object_id = Column(key_types("Favorite"))
...
class Alert(TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    name = Column(db.String(255))
    query_id = Column(key_types("Query"), db.ForeignKey("queries.id"))
...
class Widget(TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    visualization_id = Column(
        key_types("Visualization"), db.ForeignKey("visualizations.id"), nullable=True
    )
    ...
    options = Column(db.Text)
    dashboard_id = Column(key_types("Dashboard"), db.ForeignKey("dashboards.id"), index=True)
...
class Event(db.Model):
    ...
    user_id = Column(key_types("User"), db.ForeignKey("users.id"), nullable=True)
...
class ApiKey(TimestampMixin, GFKBase, db.Model):
    ...
    created_by_id = Column(key_types("User"), db.ForeignKey("users.id"), nullable=True)
...
class AlertSubscription(TimestampMixin, db.Model):
    ...
    destination_id = Column(
        key_types("NotificationDestination"), db.ForeignKey("notification_destinations.id"), nullable=True
    )
    destination = db.relationship(NotificationDestination)
    alert_id = Column(key_types("Alert"), db.ForeignKey("alerts.id"))
