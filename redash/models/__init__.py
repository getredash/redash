...
class QueryResult(db.Model, BelongsToOrgMixin):
    ...
    def store_result(
        ...
        queries = Query.query.filter(
            Query.query_hash == query_hash, Query.data_source == data_source
        )
        for q in queries:
...
class Query(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    def favorites(cls, user, base_query=None):
        ...
        return base_query.join(
            (
                Favorite,
                and_(Favorite.object_type == u"Query", Favorite.object_id == Query.id),
            )
    ...
    def past_scheduled_queries(cls):
        ...
        return filter(
            lambda x: x.schedule["until"] is not None
    ...
    def fork(self, user):
        ...
        forked_query = Query(
            name=u"Copy of (#{}) {}".format(self.id, self.name), user=user, **kwargs
        )
