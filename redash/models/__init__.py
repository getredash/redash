...
class DataSourceGroup(db.Model):
    ...
    data_source = db.relationship(DataSource,
                                  back_populates="data_source_groups")
...
@python_2_unicode_compatible
@generic_repr("id", "org_id", "data_source_id", "query_hash", "runtime",
              "retrieved_at")
class QueryResult(db.Model, BelongsToOrgMixin):
    ...
    def store_result(cls, org, data_source, query_hash, query, data, run_time,
        ...
        queries = Query.query.filter(Query.query_hash == query_hash,
        ...
        logging.info("Updated %s queries with result (%s).", len(query_ids),
                     query_hash)
...
def should_schedule_next(previous_iteration,
                         now,
                         interval,
                         time=None,
                         day_of_week=None,
...
class Favorite(TimestampMixin, db.Model):
    ...
    def is_favorite(cls, user, object):
        return cls.query.filter(cls.object == object,
                                cls.user_id == user).count() > 0
    ...
    def are_favorites(cls, user, objects):
        ...
        return map(
            ...
            cls.query.filter(
                cls.object_id.in_(map(lambda o: o.id, objects)),
...
@generic_repr("id", "name", "query_id", "user_id", "state",
              "last_triggered_at", "rearm")
class Alert(TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    def evaluate(self):
        ...
        if data["rows"] and self.options["column"] in data["rows"][0]:
            ...
            should_trigger = operators.get(
                self.options["op"], lambda v, t: False)
...
@python_2_unicode_compatible
@gfk_type
@generic_repr("id", "name", "slug", "user_id", "org_id", "version",
              "is_archived", "is_draft")
class Dashboard(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin,
    ...
    def all(cls, org, group_ids, user_id):
        query = (
            Dashboard.query.options(
                subqueryload(Dashboard.user).load_only(
                    "_profile_image_url", "name"), ).outerjoin(Widget).
            outerjoin(Visualization).outerjoin(Query).outerjoin(
                DataSourceGroup,
                Query.data_source_id == DataSourceGroup.data_source_id).filter(
    ...
    def search(cls, org, groups_ids, user_id, search_term):
        ...
        return cls.all(org, groups_ids, user_id).filter(
            cls.name.ilike(u"%{}%".format(search_term)))
...
class Visualization(TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    @classmethod
    def get_by_id_and_org(cls, object_id, org):
        return super(Visualization,
                     cls).get_by_id_and_org(object_id, org, Query)
...
class Widget(TimestampMixin, BelongsToOrgMixin, db.Model):
    id = Column(db.Integer, primary_key=True)
    visualization_id = Column(db.Integer,
                              db.ForeignKey("visualizations.id"),
                              nullable=True)
    visualization = db.relationship(Visualization,
                                    backref=backref("widgets",
                                                    cascade="delete"))
    text = Column(db.Text, nullable=True)
    width = Column(db.Integer)
    options = Column(db.Text)
    dashboard_id = Column(db.Integer,
...
@python_2_unicode_compatible
@generic_repr("id", "object_type", "object_id", "action", "user_id", "org_id",
              "created_at")
class Event(db.Model):
    ...
    additional_properties = Column(MutableDict.as_mutable(PseudoJSON),
                                   nullable=True,
                                   default={})
    created_at = Column(db.DateTime(True), default=db.func.now())
