...
REDIS_URL = os.environ.get(
    "REDASH_REDIS_URL", os.environ.get("REDIS_URL",
...
STATSD_USE_TAGS = parse_boolean(
    os.environ.get("REDASH_STATSD_USE_TAGS", "false"))

# Connection settings for Redash's own database (where we store the queries, results, etc)
SQLALCHEMY_DATABASE_URI = os.environ.get(
    "REDASH_DATABASE_URL",
    os.environ.get("DATABASE_URL", "postgresql:///postgres"))
SQLALCHEMY_MAX_OVERFLOW = int_or_none(
    os.environ.get("SQLALCHEMY_MAX_OVERFLOW"))
...
CELERY_BROKER_MASTER = os.environ.get("REDASH_CELERY_BROKER_MASTER")
...
CELERY_SSL_CONFIG = ({
    "ssl_cert_reqs":
    int(os.environ.get("REDASH_CELERY_BROKER_SSL_CERT_REQS",
                       ssl.CERT_OPTIONAL)),
    "ssl_ca_certs":
    os.environ.get("REDASH_CELERY_BROKER_SSL_CA_CERTS"),
    "ssl_certfile":
    os.environ.get("REDASH_CELERY_BROKER_SSL_CERTFILE"),
    "ssl_keyfile":
    os.environ.get("REDASH_CELERY_BROKER_SSL_KEYFILE"),
} if CELERY_BROKER_USE_SSL else None)

CELERY_WORKER_PREFETCH_MULTIPLIER = int(
    os.environ.get("REDASH_CELERY_WORKER_PREFETCH_MULTIPLIER", 1))
CELERY_ACCEPT_CONTENT = os.environ.get("REDASH_CELERY_ACCEPT_CONTENT",
                                       "json").split(",")
CELERY_TASK_SERIALIZER = os.environ.get("REDASH_CELERY_TASK_SERIALIZER",
                                        "json")
CELERY_RESULT_SERIALIZER = os.environ.get("REDASH_CELERY_RESULT_SERIALIZER",
                                          "json")
...
SCHEMAS_REFRESH_SCHEDULE = int(
    os.environ.get("REDASH_SCHEMAS_REFRESH_SCHEDULE", 30))
SCHEMAS_REFRESH_QUEUE = os.environ.get("REDASH_SCHEMAS_REFRESH_QUEUE",
                                       "celery")
...
# for more information.
REFERRER_POLICY = os.environ.get("REDASH_REFERRER_POLICY",
                                 "strict-origin-when-cross-origin")
...
REMOTE_USER_HEADER = os.environ.get("REDASH_REMOTE_USER_HEADER",
...
LDAP_LOGIN_ENABLED = parse_boolean(
    os.environ.get("REDASH_LDAP_LOGIN_ENABLED", "false"))
...
LDAP_DISPLAY_NAME_KEY = os.environ.get("REDASH_LDAP_DISPLAY_NAME_KEY",
                                       "displayName")
...
LDAP_SEARCH_TEMPLATE = os.environ.get("REDASH_LDAP_SEARCH_TEMPLATE",
                                      "(cn=%(username)s)")
# The schema to bind to (ex. cn=users,dc=ORG,dc=local)
LDAP_SEARCH_DN = os.environ.get("REDASH_LDAP_SEARCH_DN",
                                os.environ.get("REDASH_SEARCH_DN"))
...
RATELIMIT_ENABLED = parse_boolean(
    os.environ.get("REDASH_RATELIMIT_ENABLED", "true"))
THROTTLE_LOGIN_PATTERN = os.environ.get("REDASH_THROTTLE_LOGIN_PATTERN",
                                        "50/hour")
...
enabled_query_runners = array_from_string(
    os.environ.get("REDASH_ENABLED_QUERY_RUNNERS",
...
dynamic_settings = importlib.import_module(
    os.environ.get("REDASH_DYNAMIC_SETTINGS_MODULE",
...
enabled_destinations = array_from_string(
    os.environ.get("REDASH_ENABLED_DESTINATIONS",
...
DASHBOARD_REFRESH_INTERVALS = map(
    int,
    array_from_string(
        os.environ.get("REDASH_DASHBOARD_REFRESH_INTERVALS",
                       "60,300,600,1800,3600,43200,86400")),
)
QUERY_REFRESH_INTERVALS = map(
    int,
    array_from_string(
        os.environ.get(
...
PAGE_SIZE_OPTIONS = map(
    int,
    array_from_string(
        os.environ.get("REDASH_PAGE_SIZE_OPTIONS", "5,10,20,50,100")))
TABLE_CELL_MAX_JSON_SIZE = int(
    os.environ.get("REDASH_TABLE_CELL_MAX_JSON_SIZE", 50000))
...
# BigQuery
BIGQUERY_HTTP_TIMEOUT = int(
    os.environ.get("REDASH_BIGQUERY_HTTP_TIMEOUT", "600"))
...
SQLPARSE_FORMAT_OPTIONS = {
    "reindent":
    parse_boolean(os.environ.get("SQLPARSE_FORMAT_REINDENT", "true")),
