...
class TestOrganizationSettings(BaseTestCase):
    def test_post(self):
        ...
        rv = self.make_request('post', '/api/settings/organization', data={'auth_password_login_enabled': False},
                               user=admin)
        ...
        rv = self.make_request('post', '/api/settings/organization', data={'auth_password_login_enabled': True},
                               user=admin)
    ...
    def test_updates_google_apps_domains(self):
        admin = self.factory.create_admin()
        domains = ['example.com']
        rv = self.make_request('post', '/api/settings/organization', data={'auth_google_apps_domains': domains},
                               user=admin)
        updated_org = Organization.get_by_slug(self.factory.org.slug)
        self.assertEqual(updated_org.google_apps_domains, domains)
    
    def test_get_returns_google_apps_domains(self):
        admin = self.factory.create_admin()
        domains = ['example.com']
    ...
    def test_updates_github_apps_domains(self):
        admin = self.factory.create_admin()
        domains = ['example.com']
        rv = self.make_request('post', '/api/settings/organization', data={'auth_github_apps_domains': domains},
                               user=admin)
        updated_org = Organization.get_by_slug(self.factory.org.slug)
        self.assertEqual(updated_org.github_apps_domains, domains)

    def test_get_returns_github_apps_domains(self):
        admin = self.factory.create_admin()
        domains = ['example.com']
