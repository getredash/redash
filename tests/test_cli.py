...
class OrganizationCommandTests(BaseTestCase):
    ...
    def test_set_github_apps_domains(self):
        domains = ['example.org', 'example.com']
        runner = CliRunner()
        result = runner.invoke(manager, ['org', 'set_github_apps_domains', ','.join(domains)])
        self.assertFalse(result.exception)
        self.assertEqual(result.exit_code, 0)
        db.session.add(self.factory.org)
        self.assertEqual(self.factory.org.github_apps_domains, domains)

    def test_show_github_apps_domains(self):
        self.factory.org.settings[Organization.SETTING_GITHUB_APPS_DOMAINS] = [
            'example.org', 'example.com']
        db.session.add(self.factory.org)
        db.session.commit()
        runner = CliRunner()
        result = runner.invoke(manager, ['org', 'show_github_apps_domains'])
        self.assertFalse(result.exception)
        self.assertEqual(result.exit_code, 0)
        output = """
           Current list of Github Apps domains: example.org, example.com
           """
        self.assertMultiLineEqual(result.output,
                                  textwrap.dedent(output).lstrip())
