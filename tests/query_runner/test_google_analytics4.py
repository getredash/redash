...
from redash.query_runner.google_analytics4 import format_column_value, parse_ga_response
...
class TestParseGaResponse(TestCase):
    def test_parse_ga_response(self):
        ...
        expected_value = {'columns': [{'name': 'date', 'friendly_name': 'date', 'type': 'date'},
    ...
    def test_parse_ga_response_with_date_hour(self):
        response = {"dimensionHeaders": [{"name": "dateHour"}],
                    "metricHeaders": [{"name": "activeUsers", "type": "TYPE_INTEGER"}],
                    "rows": [{"dimensionValues": [{"value": "2023071312"}], "metricValues": [{"value": "7"}]},
                    ...
                    "rowCount": 21, "metadata": {"currencyCode": "USD", "timeZone": "Asia/Calcutta"},
                    "kind": "analyticsData#runReport"}

        expected_value = {'columns': [{'name': 'dateHour', 'friendly_name': 'dateHour', 'type': 'datetime'},
                                      {'name': 'activeUsers', 'friendly_name': 'activeUsers', 'type': 'string'}],
                          'rows': [{'dateHour': datetime.datetime(2023, 7, 13, 12, 0), 'activeUsers': '7'},
