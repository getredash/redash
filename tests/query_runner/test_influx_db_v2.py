...
def influx_table_list():
    ...
    table_2 = FluxTable()
    column_1 = FluxColumn(
        index=0, label='col_1', data_type='string', group=False,
        default_value='default_value_2')
    column_2 = FluxColumn(
        index=0, label='col_2', data_type='integer', group=False,
        default_value='default_value_2')
    column_3 = FluxColumn(
        index=1, label='col_3', data_type='float', group=False,
        default_value=3.0)
...
class TestInfluxDBv2:
    @mock.patch('redash.query_runner.influx_db_v2.InfluxDBv2.'
                '_create_cert_file')
    def test_get_influx_kwargs(self,
                               create_cert_file_mock: mock.MagicMock):
        ...
        create_cert_file_mock.side_effect = \
            lambda key: create_cert_file_return_dict[key]
    ...
    def test_run_query(self,
        ...
        query_mock = influx_db_client_mock.return_value.__enter__().\
            query_api().query
