...
4. Go back to `/opt/redash/current` and start the server and background workers with `bin/run honcho start -f Procfile.dev`.
5. Now the server should be available on your host on port 9001 and you can login with username admin and password admin.